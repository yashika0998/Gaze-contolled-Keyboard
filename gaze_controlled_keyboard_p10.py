...
# Library to play the sound added
import pyglet
import time

# Load sounds
# Three different sounds are included
...
# Load the camera in live
cap = cv2.VideoCapture(0)
# Created a white board where we are going to type our text
board = np.zeros((300, 1400), np.uint8)
board[:] = 255

# We used dlib library to detect first the face then the eyes
detector = dlib.get_frontal_face_detector()
predictor = dlib.shape_predictor("shape_predictor_68_face_landmarks.dat")

# Keyboard settings
# (600, 1000)--> width and height of the keybord
...
# Function to draw the letters on the keyboard
...
# Function to design a menu of keyboard to choose either left or the right
def draw_menu():
...
# Function to calculate the width and height between our eyes
def midpoint(p1 ,p2):
    return int((p1.x + p2.x)/2), int((p1.y + p2.y)/2)

font = cv2.FONT_HERSHEY_PLAIN
# Get blinking ratio to find if the eyes are open or closed
...
# Function to draw the red countours on the eye
...
# Function to detect the ratio of eye, if we are looking on the right or the left
...
# Counters
frames = 0
# Each time we want to light up a different letter we increase this  
letter_index = 0
# To show if we close the eye for this time frame then the key is pressed
blinking_frames = 0
# To blink the eye for a longer time if want the letter on white board
frames_to_blink = 6
# After every 9 frame we light up next letter
frames_active_letter = 9

# Text and keyboard settings
# Defining empty text in starting
text = ""
# Selection of the keyboard
keyboard_selected = "left"
last_keyboard_selected = "left"
# which side of keyboard is currently selected
select_keyboard_menu = True
keyboard_selection_frames = 0

while True:
    # reading the video frame of live video
    _, frame = cap.read()
    #frame = cv2.resize(frame, None, fx=0.8, fy=0.8)
    rows, cols, _ = frame.shape
    keyboard[:] = (26, 26, 26)
    frames += 1
    # Change video frames in gray format
